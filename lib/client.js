...
Client.prototype.droppedReceived = function (data) {
	...
	var segments = messageReg.exec(data.data.toString()) // [1] - client id || instance name; [2] - sequence inc; [3] - timestamp; [4] - logger level (optional); [5] - message
		, message;
	...
	if (isNumeric.test(message.sequence)) {
		...
		if (self.caught(message.sequence)) { // Is still in dropped queue; If no message, assume message was cleared from memory
			self.log(message);
		}
