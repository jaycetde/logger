...
function tcpRead(buffer) {
    ...
    if (end !== -1) {
        ...
        if (this._partial) {
            this._temp = new Buffer(this._partial.length + buffer.length);
            this._partial.copy(this._temp);
            buffer.copy(this._temp, this._partial.length);
            return this._partial = this._temp;
        } else {
            return this._partial = buffer;
        }
    }
